...
########################################
#Linear models for root nutrients#######
########################################

m1 = lm(live.p ~ dead.p, data = akk)
summary (m1)

m2 = lm(live.n.per ~ dead.n.per, data = akk)
summary (m2)

m3 = lm(live.n.per ~ s.cov, data = akk)
summary(m3)

m4 = lm(live.c.per ~ s.cov, data = akk)
summary(m4)
...
a <- ggplot(ak2, aes(x=Boreal.Tundra, y=live.dens,fill = Boreal.Tundra))+
  ...
  scale_fill_manual(values=c("gray40","gray75"),
...
ggsave("Figure2.tiff", width=8, height=5, units = ("in"))
...
CV.ld<- akk %>%
  ...
  summarise(
    n=n(),
    mean=mean(Fine.Dead.Live),
    sd=sd(Fine.Dead.Live),
    cv.l = (sd(Fine.Dead.Live)/mean(Fine.Dead.Live))*100
  ) %>%
  mutate( se=sd/sqrt(n))  %>%
...
ggsave("Figure3.tiff", width=8, height=5, units = ("in"))
...
ggsave("AK_figure4_.pdf", width=20, height=16, units = ("in"))
