...
temp <- read.csv("agg_temp_10cm.csv")
ak<- read.csv("ViPER_Root_Agg.csv")

dat <- merge(temp,ak, by.x= "Site", incomparables = NA)

#this code uses soil temperature aggregated to 10 cm#
...
#^These seem interesting to me but there is an outlier#
...
plot(dat$tdd,(dat$FineLive+dat$FineDead), col=dat$Boreal.Tundra, na.omit)
